...
const LOCATION_PROMPT_KEY = 'roomkhoj-location-prompt-state';
const LOCATION_DISMISS_MS = 24 * 60 * 60 * 1000;
...
export function AutoLocationUpdate() {
  ...
  const saveLocation = ({ coords }: GeolocationPosition) => {
    ...
    localStorage.setItem(LOCATION_PROMPT_KEY, JSON.stringify({ status: 'granted', at: Date.now() }));
    setOpen(false);
    setLoading(false);
  ...
  const requestLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(saveLocation, (error) => {
      setLoading(false);
      localStorage.setItem(LOCATION_PROMPT_KEY, JSON.stringify({ status: 'denied', at: Date.now() }));
      setOpen(false);
      if (error.code !== error.PERMISSION_DENIED) {
        setMessage('Location अहिले उपलब्ध भएन। पछि फेरि प्रयास गर्नुहोस्।');
      }
  ...
  useEffect(() => {
    ...
    // Never trigger the native browser prompt on page load. First show RoomKhoj's
    // own consent card, and remember dismiss/deny decisions for 24 hours.
    try {
      const raw = localStorage.getItem(LOCATION_PROMPT_KEY);
      const state = raw ? JSON.parse(raw) : null;
      if (state?.status === 'granted') return;
      if (state?.at && Date.now() - Number(state.at) < LOCATION_DISMISS_MS) return;
    } catch {
      // Invalid old state: safely show the website consent card again.
    }
    setOpen(true);
  }, []);
  ...
  return <div className="fixed inset-0 z-[100] flex items-end bg-black/45 p-4 pb-20 sm:items-center sm:pb-4 sm:justify-center">
    <section className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl">
      <button onClick={() => { localStorage.setItem(LOCATION_PROMPT_KEY, JSON.stringify({ status: 'dismissed', at: Date.now() })); setOpen(false); }} className="float-right rounded-full p-1 text-slate-400"><X className="h-5 w-5" /></button>
      ...
      <button onClick={() => { localStorage.setItem(LOCATION_PROMPT_KEY, JSON.stringify({ status: 'dismissed', at: Date.now() })); setOpen(false); }} className="mt-3 w-full py-2 text-sm font-medium text-slate-500">अहिले होइन</button>
