...
export function AdminSidebar({
  ...
  const MobileSidebar = () => (
    <Sheet open={isMobileOpen} onOpenChange={setIsMobileOpen}>
      <SheetTrigger asChild>
        <Button variant="ghost" size="icon" className="md:hidden fixed top-4 left-4 z-40 cursor-pointer">
      ...
      <SheetContent side="left" className="w-[280px] p-0">
        <div className="flex flex-col h-full bg-gradient-to-b from-gray-900 to-gray-800 text-white">
          ...
          <div className="p-4 border-t border-gray-700 space-y-2">
            <Button variant="ghost" className="w-full justify-start text-gray-300 hover:text-white hover:bg-gray-800 cursor-pointer" onClick={() => router.push("/") }>
              <Home className="h-4 w-4 mr-3" />
              <span>View Site</span>
            </Button>
            <Button variant="ghost" className="w-full justify-start text-red-400 hover:text-red-300 hover:bg-red-500/10 cursor-pointer" onClick={() => setShowLogoutDialog(true)}>
  ...
  const DesktopSidebar = () => (
    <aside className={cn("hidden md:flex flex-col h-screen sticky top-0 bg-gradient-to-b from-gray-900 to-gray-800 text-white transition-all duration-300", isCollapsed ? "w-20" : "w-64")}>
      <div className="flex items-center justify-between p-6 border-b border-gray-700">
        ...
        <Button variant="ghost" size="icon" onClick={() => setIsCollapsed(!isCollapsed)} className="text-gray-400 hover:text-white hover:bg-gray-800 cursor-pointer">
          {isCollapsed ? <ChevronRight className="h-4 w-4" /> : <ChevronLeft className="h-4 w-4" />}
        </Button>
      </div>

      <div className={cn("p-4 border-b border-gray-700", isCollapsed && "text-center")}>
      ...
      <div className="p-4 border-t border-gray-700 space-y-2">
        <TooltipProvider delayDuration={0}>
          <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="ghost" className={cn("w-full text-gray-300 hover:text-white hover:bg-gray-800 cursor-pointer", isCollapsed ? "px-2 justify-center" : "justify-start")} onClick={() => router.push("/") }>
                <Home className={cn("h-4 w-4", !isCollapsed && "mr-3")} />
                {!isCollapsed && <span>View Site</span>}
              </Button>
            </TooltipTrigger>
            {isCollapsed && mounted && <TooltipContent side="right">View Site</TooltipContent>}
          </Tooltip>

          <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="ghost" className={cn("w-full text-red-400 hover:text-red-300 hover:bg-red-500/10 cursor-pointer", isCollapsed ? "px-2 justify-center" : "justify-start")} onClick={() => setShowLogoutDialog(true)}>
                <LogOut className={cn("h-4 w-4", !isCollapsed && "mr-3")} />
                {!isCollapsed && <span>Logout</span>}
              </Button>
            </TooltipTrigger>
            {isCollapsed && mounted && <TooltipContent side="right">Logout</TooltipContent>}
          </Tooltip>
        </TooltipProvider>
      </div>
    </aside>
