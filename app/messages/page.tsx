              ...
              <div className="relative border-t border-border bg-card px-2.5 py-2 pb-[calc(0.5rem+env(safe-area-inset-bottom))] md:px-4 md:py-3">
                {showActionMenu && (
                  <div className="absolute bottom-[68px] left-2.5 z-50 w-64 overflow-hidden rounded-2xl border border-border bg-card p-2 shadow-2xl md:left-4">
                    <button type="button" onClick={()=>mediaInputRef.current?.click()} className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left hover:bg-muted"><div className="flex h-9 w-9 items-center justify-center rounded-full bg-muted"><ImageIcon className="h-5 w-5" /></div><div><p className="text-sm font-semibold">Photo / Video</p><p className="text-xs text-muted-foreground">Send media</p></div></button>
                    ...
                    <button type="button" onClick={()=>{setShowActionMenu(false); router.push("/profile/wallet");}} className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left hover:bg-muted"><div className="flex h-9 w-9 items-center justify-center rounded-full bg-muted"><WalletCards className="h-5 w-5" /></div><div><p className="text-sm font-semibold">Wallet / Release</p><p className="text-xs text-muted-foreground">Balance and release request</p></div></button>
                  </div>
                )}
