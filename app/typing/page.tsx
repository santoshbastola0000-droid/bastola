...
function VirtualKeyboard({ activeKey }: { activeKey: string }) {
  return <div className="relative mx-auto mt-6 block max-w-3xl select-none space-y-1 overflow-hidden rounded-2xl bg-slate-100 p-2 pb-24 sm:space-y-1.5 sm:p-3 sm:pb-24">{KEY_ROWS.map((row, rowIndex) => <div key={rowIndex} className="relative z-10 flex justify-center gap-0.5 sm:gap-1.5">{row.map((key) => <span key={key} className={`grid h-8 min-w-6 flex-1 place-items-center rounded-md border border-b-2 text-[9px] font-black uppercase transition sm:h-10 sm:min-w-10 sm:flex-none sm:rounded-lg sm:text-xs ${activeKey === key ? "border-red-600 bg-red-600 text-white -translate-y-0.5 shadow-lg shadow-red-200" : "border-slate-300 bg-white/95 text-slate-600"}`}>{key}</span>)}</div>)}<div className="relative z-10 flex justify-center"><span className={`mt-0.5 grid h-8 w-40 place-items-center rounded-lg border border-b-2 text-[9px] font-black uppercase sm:h-9 sm:w-64 sm:text-[10px] ${activeKey === " " ? "border-red-600 bg-red-600 text-white" : "border-slate-300 bg-white/95 text-slate-500"}`}>Space</span></div><div className="pointer-events-none absolute inset-x-0 bottom-[-27px] z-20"><FingerGuide activeKey={activeKey} overlay /></div></div>;
}
